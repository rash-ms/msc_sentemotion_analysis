...
# %% ##=== LOAD & PREPARE DATA ===
def prep_ml_data(df, x_col, y_col, skip_svd=False):
    ...
    vectorizer_path = "../save_models/ml_vectorizer.pkl"
...
#%% #=== WORKFLOW ===
def run_ml_workflow(df, x_col, y_col, model_name):
    ...
    prep_ml_data(df, x_col, y_col, skip_svd=skip_svd)
...
#%% #=== prep_dl_data ===
def prep_dl_data(df, x_col, y_col, use_sample=True):
    global num_classes
    ...
    # Save vectorizer if it doesn't exist
    tokenizer_path = "../save_models/dl_tokenizer.pkl"
    if not os.path.exists(tokenizer_path):
        os.makedirs(os.path.dirname(tokenizer_path), exist_ok=True)
        joblib.dump(tokenizer_path, tokenizer_path)
...
def run_dl_workflow(df, x_col, y_col, model_name):
    global num_classes

    # === Load and prepare data ===
    data = prep_dl_data(df, x_col, y_col, use_sample=True)
